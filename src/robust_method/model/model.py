...
def build_paper_config(img_size, num_classes):
    config = ConfigDict()
...
class StdConv2d(nn.Conv2d):
    def forward(self, x):
        weight = self.weight
        variance, mean = torch.var_mean(weight, dim=[1, 2, 3], keepdim=True, unbiased=False)
...
class ResNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        for index in range(len(self.body) - 1):
            x = self.body[index](x)
            right_size = int(in_size / 4 / (index + 1))
            if x.size(2) != right_size:
                pad = right_size - x.size(2)
                if not (0 < pad < 3):
                    raise RuntimeError(f"Unexpected feature shape {tuple(x.shape)} for target size {right_size}")
                feat = torch.zeros((batch_size, x.size(1), right_size, right_size), device=x.device)
...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.dropout(x), features
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.encoder(embedding_output), features
...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        if config.n_skip != 0:
            ...
            for index in range(4 - config.n_skip):
                skip_channels[3 - index] = 0
        else:
    ...
    def forward(self, hidden_states, features=None):
        ...
        height = width = int(np.sqrt(n_patch))
        x = hidden_states.permute(0, 2, 1).contiguous().view(batch_size, hidden, height, width)
        x = self.conv_more(x)
        for index, decoder_block in enumerate(self.blocks):
            skip = features[index] if features is not None and index < len(features) else None
...
# **************************************************************************** #
#                                                                              #
#                           PAPER TRANSUNET WRAPPER                            #
#                                                                              #
# **************************************************************************** #


class PaperTransUNet(nn.Module):
...
def load_checkpoint_state_dict(model_path, device):
    try:
...
def infer_num_classes(state_dict, default=2):
    for key in ("segmentation_head.0.weight", "module.segmentation_head.0.weight"):
        if key in state_dict:
            return int(state_dict[key].shape[0])
    return default


def build_paper_transunet(model_path, img_size=224, device=None, num_classes=None):
    device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")
    state_dict = load_checkpoint_state_dict(model_path, device)
    num_classes = num_classes or infer_num_classes(state_dict, default=2)
    config = build_paper_config(img_size=img_size, num_classes=num_classes)
    model = PaperTransUNet(config, img_size=img_size).to(device)

    errors = []
    for variant in candidate_state_dicts(state_dict):
        try:
            missing, unexpected = model.load_state_dict(variant, strict=False)
            if missing or unexpected:
                errors.append(f"missing={len(missing)}, unexpected={len(unexpected)}")
                continue
    ...
    raise RuntimeError(
        f"Unable to load checkpoint `{model_path}` into paper TransUNet. Attempts: {errors}"
    )
