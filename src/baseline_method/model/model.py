...
class ViT(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.mlp_head(x[:, 0, :]) if self.classification else x[:, 1:, :]
...
class EncoderBottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, base_width=64):
        ...
        self.conv2 = nn.Conv2d(width, width, kernel_size=3, stride=2, groups=1, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.relu(self.norm3(self.conv3(x)) + x_down)
        return x
...
class TransUNet(nn.Module):
    def __init__(self, img_dim=224, in_channels=3, out_channels=128, head_num=4, mlp_dim=512, block_num=8, patch_dim=16, class_num=1):
        super().__init__()
...
if __name__ == '__main__':
    model = TransUNet(img_dim=224, class_num=1)
    print(f"Paramètres: {sum(p.numel() for p in model.parameters())}")
    print(f"Output shape: {model(torch.randn(2, 3, 224, 224)).shape}")
