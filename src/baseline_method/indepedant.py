...
python3 src/baseline_method/threshold_test.py \
...
def checkpoint_num_classes(state_dict):
    for key in ("decoder.conv1.weight", "module.decoder.conv1.weight"):
...
def build_model(model_path, img_size, fallback_num_classes):
    ...
    num_classes = checkpoint_num_classes(state_dict) or fallback_num_classes
    model = TransUNet(img_dim=img_size, class_num=num_classes).to(DEVICE)
    ...
    raise RuntimeError(
        f"Unable to load checkpoint `{model_path}` into TransUNet. Attempts: {errors}"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate a trained TransUNet checkpoint on the independent split."
    )
